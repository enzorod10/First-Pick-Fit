...
import MobileWorkout, { MobileDraggableWorkout } from './Workout/MobileWorkout';
...
export const MobileWorkoutsComponent = ({ workouts, userId }: { workouts: Workout[], userId: string | undefined}) => {
    ...
    const [newWorkout, setNewWorkout] = useState<HTMLDivElement | null>(null)
    
    const dispatch = useDispatch();

    const initiateEditMode = (workout?: Workout) => {
    ...
    const handleEditedWorkout = (workout?: Workout) => {
        setIsEditingWorkout(false)
            ...
            <div className={styles.workoutsContainer}>
                ...
                <div className={styles.workoutsInnerContainer}>
                    {search.trim() === '' ? workouts.map(workout => {
                        return(
                            <MobileDraggableWorkout key={workout.id} workout={workout}>
                                <MobileWorkout workout={workout} initiateEditMode={initiateEditMode}/>
...
const WorkoutEditor = ( { handleCreatedWorkout, handleEditedWorkout, workout, userId }: { handleCreatedWorkout?: (workout?: Workout) => void, handleEditedWorkout?: (workout?: Workout) => void, workout?: Workout, userId: string | undefined }) => {
