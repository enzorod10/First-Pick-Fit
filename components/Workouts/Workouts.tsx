                ...
                <div ref={setNodeRef} className={styles.workoutExercisesCreatorContainer}>
                                ...
                                return (
                                    <div key={exercise.id} ref={el => workoutExercisesRef.current[i] = el} className={styles.individualWorkoutExercises} > 
                                        <div style={{fontSize: '0.8rem', width: '90%', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center'}}>
                                            ...
                                            {newSetMode.id !== exercise.id && <button onClick={() => (setNewSetMode({...newSetMode, id: exercise.id}), workoutExercisesRef.current[i]?.scrollIntoView({ behavior: 'smooth', block: 'center', inline: 'nearest' }))} className="text-white bg-gray-800 hover:bg-gray-900 focus:outline-none focus:ring-4 focus:ring-gray-300 font-medium rounded-lg px-2 py-0.5 text-xs dark:bg-gray-800 dark:hover:bg-gray-700 dark:focus:ring-gray-700 dark:border-gray-700">Add Sets</button>}
                                        </div>
                                        <div style={{display: 'flex', flexDirection: 'column', width: '100%', gap: '2px'}}>
                                            ...
                                            <div style={{display: 'flex', flexDirection: 'column', width: '100%', fontSize: '0.7rem', borderRadius: '2px', border: '1px var(--oxford-blue) solid'}}>
                                                ...
                                                <div style={{display: 'flex', flexDirection: 'row', justifyContent: 'space-around', padding: '0.3rem'}}>
                                                    <button className="text-gray-900 hover:text-white border border-gray-800 hover:bg-gray-900 focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium rounded-lg px-2 py-0.5 text-xs text-center dark:border-gray-600 dark:text-gray-400 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-800" onClick={() => setNewSetMode({id: '', data: { sets: 3, reps: 8, weight: 100, id: '' }})} style={{ cursor: 'pointer', color: 'var(--charcoal)', minWidth: 'max-content' }}>Cancel</button>
                                                    <button className="text-blue-700 hover:text-white border border-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg px-2 py-0.5 text-xs text-center dark:border-blue-500 dark:text-blue-500 dark:hover:text-white dark:hover:bg-blue-500 dark:focus:ring-blue-800" onClick={() => handleNewSet(exercise.id)} style={{ cursor: 'pointer', color: 'var(--charcoal)', minWidth: 'max-content' }}>Add</button> 
                                                </div>
                                            </div>}
                                        </div>
                    ...
                    <div style={{border: '1px var(--oxford-blue) solid', backgroundColor: successfullyDropped ? 'white' : 'var(--charcoal)', opacity: '0.1'}} className={`${styles.individualWorkoutExercises} ${successfullyDropped ? styles.active : ''}`}>
                        <div className={`${successfullyDropped ? styles.active : ''}`} style={{ opacity: '0', fontSize: '0.8rem', width: '90%', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center'}}>
                            <div>{ghostPiece}</div>
                            <button className="text-white bg-gray-800 hover:bg-gray-900 focus:outline-none focus:ring-4 focus:ring-gray-300 font-medium rounded-lg px-2 py-0.5 text-xs dark:bg-gray-800 dark:hover:bg-gray-700 dark:focus:ring-gray-700 dark:border-gray-700">Add Sets</button>
                        </div>
                        <div style={{display: 'flex', flexDirection: 'column', width: '100%', gap: '2px'}}>
                        </div>
